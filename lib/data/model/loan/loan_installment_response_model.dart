...
class LoanInstallmentResponseModel {
  LoanInstallmentResponseModel({String? remark, String? status, Message? message, MainData? data}) {
    _remark = remark;
    _status = status;
    _message = message;
    _data = data;
    ...
    if (json['application_form'] != null) {
      ...
      try {
        if (json['application_form'] is List<dynamic>) {
          ...
          List<ApplicationForm>? list = map.entries.map((e) => ApplicationForm(name: e.key ?? '', type: e.value['type'], value: e.value['value'])).toList();
          if (list.isNotEmpty) {
...
class Installments {
  Installments({List<Data>? data, dynamic nextPageUrl, String? path, String? total}) {
    _data = data;
