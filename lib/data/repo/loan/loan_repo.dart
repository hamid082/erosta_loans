...
class LoanRepo {
  ...
  Future<ResponseModel> getMyLoanList(int page, String status, String loanNumber) async {
    //?search=sdfasdfdsf
    String srcUrl = "${UrlContainer.baseUrl}${UrlContainer.myloanListUrl}?search=$loanNumber";

    String url = status == '999' ? "${UrlContainer.baseUrl}${UrlContainer.myloanListUrl}?page=$page" : "${UrlContainer.baseUrl}${UrlContainer.myloanListUrl}?page=$page&status=$status";
    log(url);
    ResponseModel responseModel = await apiClient.request(loanNumber.isNotEmpty ? srcUrl : url, Method.getMethod, null, passHeader: true);
    return responseModel;
  }
